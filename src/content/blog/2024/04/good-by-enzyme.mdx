...
If you're only simulating a "click" you're missing out on a lot of the interactions that a user would have. This leads
...
`fireEvent` method though if you really want to). This takes into account the multitidue events that could happen when a user
...
Goodbye Enzyme, you will not be missed.
