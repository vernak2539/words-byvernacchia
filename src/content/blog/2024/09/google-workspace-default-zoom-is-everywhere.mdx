...
I think the thing that took me the longest time was figuring out how to sign up to become a Microsoft Edge Add-on developer. 😅 It
...
This is the same version used by Chrome. I won't have to do much to make this work!" Easy, right?!? Sure...

I started using Manifest V3. All the docs said to define your Add-on's ID in the manifest. So, I went
...
It was relatively easy to extend for Firefox and Edge, just requiring the additional builds and packaging of the extension for these new targets.
...
This makes publishing a breeze. All I do is publish a new version using Git tags and the GitHub Actions takes care of the rest.
...
I originally set Sentry up to work just with Chrome. Specifically, I only uploaded source maps for the Chrome version of the extension. But, since
...
Thankfully, this is possible using Sentry's GitHub action! One of my concerns was that the release would be created by during the first upload, then
in subsequent uploads it would fail due to trying to create a release that already exists. I had to test it and cross my fingers it worked.

It ended up working! So now all the source maps for each build are associated to single Sentry release. [My GitHub action](https://github.com/vernak2539/chrome-extension-google-doc-default-zoom/blob/main/.github/workflows/submit.yml#L40-L66)
